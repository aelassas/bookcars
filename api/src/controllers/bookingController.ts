...
//@ts-ignore
            ...
            strings.setLanguage(getUserLang(user))
        ...
        const locale = getUserLang(user) === 'fr' ? 'fr-FR' : 'en-US'
        const options = {
            weekday: 'long',
            month: 'long',
            year: 'numeric',
        ...
        //@ts-ignore
        pickupLocation.name = pickupLocation.values.filter(value => value.language === getUserLang(user))[0].value
...
export const updateStatus = async (req: Request, res: Response) => {
    try {
        const {ids: _ids, status} = req.body, ids = _ids.map((id: unknown) => new mongoose.Types.ObjectId(String(id)))
...
export const deleteBookings = async (req: Request, res: Response) => {
    try {
        ...
        Booking.deleteMany({_id: {$in: ids}}, async (err: unknown) => {
            if (err) {
...
export const hasBookings = (req: Request, res: Response) => {
...
export const bookingsMinDate = (req: Request, res: Response) => {
...
export const bookingsMaxDate = (req: Request, res: Response) => {
