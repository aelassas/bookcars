...
export const getCars = async (data, page, size) =>
  axios
    .post(`${Env.API_HOST}/api/frontend-cars/${page}/${size}}`, data)
    .then((res) => res.data)

export const getCar = async (id) => {
  const language = await UserService.getLanguage()
  return axios
    .get(`${Env.API_HOST}/api/car/${encodeURIComponent(id)}/${language}`)
    .then((res) => res.data)
}
...
export const getBookingCars = async (keyword, data, page, size) => {
  ...
  return axios
    .post(
      `${Env.API_HOST}/api/booking-cars/${page}/${size}/?s=${encodeURIComponent(
        keyword,
      )}`,
      data,
      { headers: authHeader },
