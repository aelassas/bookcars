...
const CarList = (props) => {
  ...
  const getExtraIcon = (option, extra) => {
    ...
    if (props.booking) {
      ...
      if (
        ...
        props.booking.collisionDamageWaiver &&
        extra > 0
      )
        available = true
      if (
        option === 'theftProtection' &&
        props.booking.theftProtection &&
        extra > 0
      )
        available = true
      if (
        option === 'fullInsurance' &&
        props.booking.fullInsurance &&
        extra > 0
      )
        available = true
      if (
        option === 'additionalDriver' &&
        props.booking.additionalDriver &&
        extra > 0
      )
        available = true
    }
  ...
  return (
    <>
      <section
        className={`${props.className ? `${props.className} ` : ''}car-list`}
      >
        {rows.length === 0
          ? !loading &&
            !props.loading && (
                ...
                <div className='car'>
                  ...
                  {!props.hideCompany && (
                    <div className='car-company'>
                      ...
                      <label className='car-company-info'>
                ...
                {!props.hidePrice && (
                  <div className='action'>
                    <Button
                      ...
                      href={`/checkout?c=${car._id}&p=${
                        props.pickupLocation
                      }&d=${
                        props.dropOffLocation
                      }&f=${props.from.getTime()}&t=${props.to.getTime()}`}
                    >
                      {strings.BOOK}
                    </Button>
                  </div>
