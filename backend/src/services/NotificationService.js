...
export const markAsRead = (userId, ids) =>
  axios
    .post(
      `${Env.API_HOST}/api/mark-notifications-as-read/${encodeURIComponent(
        userId,
      )}`,
      { ids },
      { headers: UserService.authHeader() },
...
export const markAsUnread = (userId, ids) =>
  axios
    .post(
      `${Env.API_HOST}/api/mark-notifications-as-unread/${encodeURIComponent(
        userId,
      )}`,
      { ids },
      { headers: UserService.authHeader() },
...
export const getNotifications = (userId, page) =>
  axios
    .get(
      `${Env.API_HOST}/api/notifications/${encodeURIComponent(
        userId,
      )}/${page}/${Env.PAGE_SIZE}`,
      { headers: UserService.authHeader() },
    )
