...
const BookingList = (props) => {
    ...
    const handleConfirmUpdate = async () => {
        try {
            ...
            const s = await BookingService.updateStatus(data)
            assert(s === 200, "BookingService could not update status");
            rows.forEach(row => {
    ...
    const handleConfirmDelete = async () => {
        try {
            if (Env.isMobile()) {
                ...
                assert(status === 200, "BookingService could not delete booking");
                ...
                // todo: this line looks like a bug
    ...
    return (
        <div className='bs-list'>
                                ...
                                return (
                                    <div key={booking._id} className='booking-details'>
                                        ...
                                        <div className='booking-detail' style={{height: bookingDetailHeight}}>
                                            ...
                                            <div className='booking-detail-value'>
                                                <Link
                                                    href={`user/?u=${booking.driver._id}`}>{booking.driver.fullName}</Link>
                                            </div>
                                        </div>
                                        <div className='booking-detail' style={{height: bookingDetailHeight}}>
            ...
            >
                ...
                <DialogContent
                    className='dialog-content'>{selectedIds.length === 0 ? strings.DELETE_BOOKING : strings.DELETE_BOOKINGS}</DialogContent>
                <DialogActions className='dialog-actions'>
